        ...
        src(`${config.entry}/pages/**/*.+(html|nunjucks|njk)`)
            ...
            .pipe(
                nunjucksRender({
                    ...
                    data: {
                        ...config.njkOpt,
                        IMG_PATH: isProduction ? `${config.imgUrl}assets/images/` : `${assetsPath}/images/`,
...
const build = series(cleanProd, parallel(generateCSS, bundleJS, nunjucksTask), copyAssets);
const mergeSrlJS = series(compressMainJS, mergeJS, cleanTemp);

// 將默認任務導出為 ES 模塊
export default isProduction ? build : series(parallel(generateCSS, bundleJS, nunjucksTask), browserSync);
